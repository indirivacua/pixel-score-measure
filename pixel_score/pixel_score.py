...
class PixelScore(Metric):
    ...
    def _pad_history(self, history: List[tuple], max_length: int, batch_idx: int) -> List[tuple]:
    ...
    def _stop_condition(self, mode: str, current: float, target: float) -> bool:
        match mode:
            case "erode": return current <= target
            case "dilate": return current >= target
            case _: raise ValueError("Invalid morphology operation")
    ...
    def update(
        ...
        self.output_curves = self._batch_morphology(mode, target_fraction, threshold, max_iter, callbacks)
    ...
    def compute(self) -> torch.Tensor:
        ...
        x = torch.gather(self.output_curves[:, :, 0], 1, torch.argsort(self.output_curves[:, :, 0], dim=1))
        y = torch.gather(self.output_curves[:, :, 1], 1, torch.argsort(self.output_curves[:, :, 0], dim=1))
